 ...
 */
export async function getNotesByNotepad(notepadId, limit = 10, page = 1) {
    try {
 ...
 */
export function validateNoteCreation(data) {
...
export async function updateNote(id, updateData) {
    try {
        ...
        if (updateData.content)
            sanitizedData.content = xss(updateData.content);
 ...
 */
export async function noteBelongsToNotepad(noteId, notepadId) {
    try {
        const note = await prisma.note.findUnique({
            where: { id: noteId },
