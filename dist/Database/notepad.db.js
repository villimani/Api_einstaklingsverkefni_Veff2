 ...
 */
export async function getNotepads(limit = 10, page = 1, ownerId) {
    try {
 ...
 * @returns {Promise<{ notepads: Array<Notepad>, total: number, page: number, limit: number }>}
 */
export async function getPublicNotepads(limit = 10, page = 1) {
    try {
        const offset = (page - 1) * limit;
        const notepads = await prisma.notepad.findMany({
            where: { isPublic: true },
 ...
 */
export function validateNotepadCreation(data) {
 ...
 */
export function validateNotepadUpdate(data) {
 ...
 */
export async function createNotepad(notepadData) {
    try {
        const sanitizedData = {
 ...
 */
export async function updateNotepad(id, updateData) {
    try {
        const sanitizedData = {};
        if (updateData.title)
            sanitizedData.title = xss(updateData.title);
 ...
 */
export async function userOwnsNotepad(notepadId, userId) {
    try {
        const notepad = await prisma.notepad.findUnique({
            where: { id: notepadId },
            select: { ownerId: true },
